...
public class EquipmentService extends SuperService {
  ...
  @Autowired
  public EquipmentService(final EquipmentRepo equipmentRepo,
  ...
  public String equipmentList(final Model model,
    ...
    if (equipmentCategory == null
        || equipmentSex == null
        || (equipments == null || equipments.size() == 0)) {
  ...
  public String viewEquipment(final Model model,
    ...
    final List<Review> reviews = reviewRepo.findReviewsByEquipmentId(equipmentId);
  ...
  private String equipmentShortList(final Model model) {
    List<Equipment> equipments = equipmentRepo.findAll();

    if (equipments.size() < 5 && equipments.size() != 0) {
      model.addAttribute("equipmentShortList", equipments);
      model.addAttribute("messageEmpty", false);
    } else if (equipments.size() > 4) {
  ...
  public String searchEquipmentList(final Model model,
    ...
    if (searchBy != null && searchBy.length() == 0) {
      ...
      final String searchByForQuery = "%" + searchBy + "%";
      equipments = equipmentRepo.findEquipmentByNameLikeOrLongDescriptionLikeOrShortDescriptionLike(searchByForQuery,
          searchByForQuery, searchByForQuery);
    }
